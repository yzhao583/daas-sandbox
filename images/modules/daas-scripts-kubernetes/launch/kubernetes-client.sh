...
query_route_host() {
    ...
    if [ "${routeName}" != "" ]; then
        ...
        if [ "${response: -3}" = "200" ]; then
            ...
            host=$(echo ${response::- 3} | python3 -c 'import json,sys;obj=json.load(sys.stdin);print (obj["spec"]["host"])')
        else
...
query_route_protocol() {
    ...
    if [ "${routeName}" != "" ]; then
        ...
        if [ "${response: -3}" = "200" ]; then
            # parse the json response to get the route host
            local protocol=$(echo ${response::- 3} | python3 -c 'import json,sys;obj=json.load(sys.stdin); protocol = "https" if "tls" in obj["spec"] else "http" ; print (protocol)')
        else
...
query_route_service() {
    ...
    if [ "${routeName}" != "" ]; then
        ...
        if [ "${response: -3}" = "200" ]; then
            ...
            service=$(echo ${response::- 3} | python3 -c 'import json,sys;obj=json.load(sys.stdin);print (obj["spec"]["to"]["name"])')
        else
